...
const checkPostSig = async (_post) => {
    const queryResults = await knex.select(`modulus`).from('users_lite').where({
...
const checkSignator = async (_post) => {
    const _res = await knex('groups')
        .where({ group_name: _post.data.group_name })
        .select('public');
    const publicity = _res[0].public;
    const queryResults = await knex.select(`modulus`).from('users').where({
